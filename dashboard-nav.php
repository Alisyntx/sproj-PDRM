
    <nav class="side-bar close" id="side-bar-js">
        <header>
            <div class="image-text">
                <span class="image">
                    <img src="image/logofin.png" alt="wuuwuw">
                </span>
                <div class="text header-text">
                    <span class="name">Pahanocoy</span>
                    <span class="professional">PDRM</span>
                </div>
            </div>
            <i class='bx bx-chevron-right togglei'></i>
        </header>
        <div class="menu-bar">
            <div class="menu">
                <ul class="menu-link">
                    <li class="search-box">
                        <i class='bx bx-search-alt iconi'></i>
                        <input type="search" placeholder="Search...">
                    </li>
                    <li class="nav-link ">
                        <a href="#homeNav" id="main-dash">
                        <i class='bx bx-home-heart iconi'></i>
                        <span class="text nav-text">dashboard</span>
                        </a>
                    </li>
                    <li class="nav-link ">
                        <a href="#announNav" id="ann-dash">
                        <i class='bx bxs-comment-detail iconi'></i>
                        <span class="text nav-text">Announcement</span>
                        </a>
                    </li>
                    <li class="nav-link " id="weather-dash">
                        <a href="#centerNav" id="weather-dash">
                        <i class='bx bxs-location-plus iconi'></i>
                        <span class="text nav-text">Center</span>
                        </a>
                    </li>
                    <li class="nav-link ">
                        <a href="#weatherNav" id="user-dash">
                        
                        <i class='bx bx-radar iconi'></i>
                        <span class="text nav-text">Weather</span>
                        </a>
                    </li>
                    <li class="nav-link" id="center-dash">
                        <a href="#reportNav">
                        <i class='bx bxs-report iconi'></i>
                        <span class="text nav-text">Report</span>
                        </a>
                    </li>
                    <li class="nav-link " id="weather-dash">
                        <a href="#userNav" id="weather-dash">
                        <i class='bx bxs-user-account iconi'></i>
                        <span class="text nav-text">User</span>
                        </a>
                    </li>
                    <li class="nav-link " id="weather-dash">
                        <a href="#genrepNav">
                        <i class='bx bxs-food-menu iconi'></i>
                        <span class="text nav-text">User</span>
                        </a>
                    </li>
                </ul>
            </div>
            <div class="bottom-content">
                <li class="">
                    <a href="php/logout.php">
                    <i class='bx bx-log-out iconi'></i>
                    <span class="text nav-text">log-out</span>
                    </a>
                </li>
                <li class="mode">
                    <div class="moon-sun">
                    <i class='bx bx-moon iconi moon'></i>
                    <i class='bx bx-sun iconi sun'></i>
                    </div>
                    <span class="mode-text text">Dark Mode</span>
                    <div class="toggle-switch">
                        <span class="switch"></span>
                    </div>
                </li>
            </div>
        </div>
    </nav>
    